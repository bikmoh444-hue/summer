...
export default async function ReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="summer-site min-h-screen text-white" style={{ '--site-bg': "url('/beach-summer.png')" } as React.CSSProperties}>
      <div className="summer-overlay grid min-h-screen place-items-center px-4 py-8">
              ...
              <section className="card-glass p-5">
                ...
                <div className="space-y-3">
                  {(order.order_items ?? []).map((item) => (
                    <div key={item.id} className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-primary">{formatMad(Number(item.total_price))}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-5 space-y-2 border-t border-white/15 pt-4 text-sm">
                  ...
                  <div className="flex items-end justify-between pt-2">
                    ...
                    <span className="price-text text-2xl">{formatMad(Number(order.total))}</span>
                  </div>
                </div>
              </section>
